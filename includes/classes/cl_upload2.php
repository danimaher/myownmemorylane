...
class ImgUploader
{
  ...
  function secure()
  {
    ...
    if(filesize($this->tmp_name) > 1024*1024*1024*10) // allows for five megabytes.  Change this number if need be.
    {
  ...
  {
    return $this->upload($folder, $name, "600", "400");
  }

  function upload($folder, $name, $width, $height, $scaleUp = true)
  {
...
if (file_exists("userfiles/".$name.$ext)) {
	
	$name=$name.rand();
	$savename="userfiles/".$name.$ext;
